...
namespace MySample
{
    ...
    public class PrefabTest : MonoBehaviour
    {
        ...
        private float tileTimer = 1f; // 타이머 기준 시간
        ...
        private void Start()
        {
            ...
            // [2] 위치 지정해서 (5, 0.05, 8) 에 프리팹 게임 오브젝트의 사본 제작
            ...
            // [3] 맵 타일(가로(10) * 세로(10), 타일 간 간격 5)
            // GenenrateMapTile(10, 10);   
            // GenenrateMapTile(10, 10, this.transform);   // 부모 지정
            ...
            // [5] 타이머 초기화(-로 내려가는 타이머)
            countdown = tileTimer;

            // [6] 랜덤 위치에 타일을 1초마다 1개씩 찍기 - 10개찍기
            // 타일 1개 찍고 1초 딜레이 -> 타일 1개 찍고 -> 1초 딜레이
            // 코루틴 함수 호출 : StartCoroutine(코루틴함수이름())
            Debug.Log("[1]타일 생성 준비");
        ...
        private void Update()
        {
            // [5] 랜덤 위치에 타일을 1초마다 1개씩 찍어서 모두 10개 찍기
        ...
        // 맵 타일 찍는 함수, 매개변수 가로, 세로 타일 갯수를 입력받아 맵타일 찍는 함수
        void GenenrateMapTile(int row, int column)
        {
            for (int i = 0; i < row; i++)
            {
        ...
        // 매개변수로 생성되는 맵타일의 부모오브젝트, 가로, 세로 타일 갯수를 입력받아 맵타일 찍는 함수
        void GenenrateMapTile(int row, int column, Transform parent)
        {
            for (int i = 0; i < row; i++)
            {
                for (int j = 0; j < column; j++)
